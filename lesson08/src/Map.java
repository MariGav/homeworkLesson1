...
public class Map extends JPanel {
    ...
    private void gameLogic(MouseEvent mouse) {
        ...
        if (!isValidCell(cellX, cellY) || !isEmptyCell(cellX, cellY)) {
            gameWindow.saveLog("Invalid move to [" + (cellX + 1) + ":" + (cellY + 1) + "]");
            return;
        }

        map[cellY][cellX] = HUMAN;
        gameWindow.saveLog("Human turn to [" + (cellX + 1) + ":" + (cellY + 1) + "]");
    ...
    private void aiTurn() {
        ...
        map[y][x] = AI;
        gameWindow.saveLog("AI turn to [" + (x + 1) + ":" + (y + 1) + "]");
    }
        ...
        for (int i = 0; i < mapSizeY; i++) {
            for (int j = 0; j < mapSizeX; j++) {
                if (isEmptyCell(j, i)) {
                    map[i][j] = AI;
                    if (checkWin(AI)) {
                        gameWindow.saveLog("AI turn to [" + (j + 1) + ":" + (i + 1) + "]");
                        return true;
                    }
                    map[i][j] = EMPTY_FIELD;
                }
